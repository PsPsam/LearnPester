...
# -Modules ActiveDirectory, GroupPolicy, SqlServer
	...
	if ($Action -eq 'Install') 
	{
		if ($feature = Get-WindowsFeature -ComputerName $ComputerName -Name $Name -ErrorAction Ignore) 
		{
			if ($feature.installed)
			{
				...
				Write-Output "$ComputerName has feature $name installed"
			}
			else 
			{
				...
				try {
					...
					Write-Output "$ComputerName got feature $name installed"
				}
				catch {
					Write-Output "$ComputerName failed to get feature $name installed"
				}
			}
		}
	}
		...
		if ($feature = Get-WindowsFeature -ComputerName $ComputerName -Name $Name -ErrorAction Ignore) 
		{
			if ($feature.installed)
			{
				...
				Try {
					...
					Write-Output "$ComputerName got feature $name removed"
				}
				catch {
					Write-Output "$ComputerName failed to remove feature $name"
				}
...
Function Test-SQL {
	...
	if (Get-Service -Name $sqlservices -ComputerName $ComputerName) {
		$true
	}
	else {
		$false
